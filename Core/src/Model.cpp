...
std::vector<Texture> Model::LoadTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		// If the texture is already loaded, skip the load.
		for (unsigned int j = 0; j < _loadedTextures.size(); j++)
		{
			if (std::strcmp(_loadedTextures[j].path.data(), str.C_Str()) == 0)
			{
				textures.push_back(_loadedTextures[j]); // a texture with the same filepath has already been loaded, continue to next one. (optimization)
				skip = true;
				break;
			}
		}
